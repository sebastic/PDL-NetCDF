...
/*
 * Initialize a perl scalar value from a vector.
 * The scalar value must alreay exist!
 */
static int
sv_initvec1(sv, vec)
    SV		*sv;
    Vector	*vec;
{
    ...
    case IT_CHAR:
      {
	sv_setpvn(sv, vec->data, (int)vec->nelt);
	break;
      }
    case IT_SHORT:
      {
	sv_setpvn(sv, vec->data, (int)vec->nelt*2);
	break;
      }
    case IT_INT:
      {
	sv_setpvn(sv, vec->data, (int)vec->nelt*4);
	break;
      }
    case IT_NCLONG:
      {
    ...
    case IT_DOUBLE:
      {
	sv_setpvn(sv, vec->data, (int)vec->nelt*8);
	break;
      }
    }
...
int arg;
{
	...
	break;
    case 'E':
	if (strEQ(name, "NC_EBADDIM"))
	    return NC_EBADDIM;
	if (strEQ(name, "NC_EBADID"))
	    return NC_EBADID;
	if (strEQ(name, "NC_EBADTYPE"))
	    return NC_EBADTYPE;
	if (strEQ(name, "NC_EEXIST"))
	    return NC_EEXIST;
	if (strEQ(name, "NC_EGLOBAL"))
	    return NC_EGLOBAL;
	if (strEQ(name, "NC_EINDEFINE"))
	    return NC_EINDEFINE;
	if (strEQ(name, "NC_EINVAL"))
	    return NC_EINVAL;
	if (strEQ(name, "NC_EINVALCOORDS"))
	    return NC_EINVALCOORDS;
	if (strEQ(name, "NC_EMAXATTS"))
	    return NC_EMAXATTS;
	if (strEQ(name, "NC_EMAXDIMS"))
	    return NC_EMAXDIMS;
	if (strEQ(name, "NC_EMAXNAME"))
	    return NC_EMAXNAME;
	if (strEQ(name, "NC_EMAXVARS"))
	    return NC_EMAXVARS;
	if (strEQ(name, "NC_ENAMEINUSE"))
	    return NC_ENAMEINUSE;
	if (strEQ(name, "NC_ENFILE"))
	    return NC_ENFILE;
	if (strEQ(name, "NC_ENOTATT"))
	    return NC_ENOTATT;
	if (strEQ(name, "NC_ENOTINDEFINE"))
	    return NC_ENOTINDEFINE;
	if (strEQ(name, "NC_ENOTNC"))
	    return NC_ENOTNC;
	if (strEQ(name, "NC_ENOTVAR"))
	    return NC_ENOTVAR;
	if (strEQ(name, "NC_ENTOOL"))
	    return NC_ENTOOL;
	if (strEQ(name, "NC_EPERM"))
	    return NC_EPERM;
	if (strEQ(name, "NC_ESTS"))
	    return NC_ESTS;
	if (strEQ(name, "NC_EUNLIMIT"))
	    return NC_EUNLIMIT;
	...
	break;
    case 'F':
	if (strEQ(name, "NC_FATAL"))
	    return NC_FATAL;
	if (strEQ(name, "NC_FILL"))
	    return NC_FILL;
	if (strEQ(name, "NC_FILL_BYTE"))
	    return FILL_BYTE;
	if (strEQ(name, "NC_FILL_CHAR"))
	    return FILL_CHAR;
	if (strEQ(name, "NC_FILL_DOUBLE"))
	    return FILL_DOUBLE;
	if (strEQ(name, "NC_FILL_FLOAT"))
	    return FILL_FLOAT;
	if (strEQ(name, "NC_FILL_LONG"))
	    return FILL_LONG;
	if (strEQ(name, "NC_FILL_SHORT"))
	    return FILL_SHORT;
	if (strEQ(name, "NC_FLOAT"))
	    return NC_FLOAT;
	break;
    case 'G':
	if (strEQ(name, "NC_GLOBAL"))
	    return NC_GLOBAL;
	break;
    case 'H':
	...
	break;
    case 'M':
	if (strEQ(name, "NC_MAX_ATTRS"))
	    return MAX_NC_ATTRS;
	if (strEQ(name, "NC_MAX_DIMS"))
	    return MAX_NC_DIMS;
	if (strEQ(name, "NC_MAX_NAME"))
	    return MAX_NC_NAME;
	if (strEQ(name, "NC_MAX_OPEN"))
	    return MAX_NC_OPEN;
	if (strEQ(name, "NC_MAX_VARS"))
	    return MAX_NC_VARS;
	if (strEQ(name, "NC_MAX_VAR_DIMS"))
	    return MAX_VAR_DIMS;
	break;
    case 'N':
	...
	break;
    case 'W':
	if (strEQ(name, "NC_WRITE"))
	    return NC_WRITE;
	break;
    case 'X':
	if (strEQ(name, "NC_XDR_D_INFINITY"))
#ifdef XDR_D_INFINITY
	    return XDR_D_INFINITY;
#else
	    goto not_there;
#endif
	if (strEQ(name, "NC_XDR_F_INFINITY"))
...
# This varput takes a pdl pointer as input
int
...
int
ncvargetscalar(ncid, varid, start, count, values)
    int		ncid
    int		varid
    SV *	start
    SV *	count
    ...
    CODE:
    {
	/* This variation of ncvarget returns not a perl list, but a scalar variable */
	...
	if (start_vec.initialized)
	{
		...
		if (ncvarget(ncid, varid, (long*)start_vec.data,
			    (long*)count_vec.data, SvPV(values, na)) != -1)
		{
		    RETVAL = 0;     /* success */
